...
for path in [metadata_file, length_file, area_file, time_file]:
    if not path.exists():
...
n_samples = int(np.fromfile(length_file, dtype=np.int64, count=1)[0])
...
if area.size != n_samples:
    raise RuntimeError(f"Area length mismatch: {area.size} != {n_samples}")
if time.size != n_samples:
    raise RuntimeError(f"Time length mismatch: {time.size} != {n_samples}")
...
output = output_dir / "minimum_time_response.png"
